...
Catch::Clara::ParserResult
parse_log_level(int const d, spdlog::level::level_enum& log_level)
{
    if (d < 0 || d > spdlog::level::off) {
        return Catch::Clara::ParserResult::runtimeError(
            "Log level must be between 0 and 6");
    } else {
        log_level = static_cast<spdlog::level::level_enum>(d);
...
int main(int argc, char** argv)
{
    ...
    scalable_ccd::logger().set_level(log_level);
