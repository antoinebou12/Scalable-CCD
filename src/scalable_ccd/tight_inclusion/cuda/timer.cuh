    ...
    Arguments... args)
{
    ...
    if (error != cudaSuccess) {
        ...
        if (err != cudaSuccess)
            printf(
                "Device-kernel launch failure %s\n", cudaGetErrorString(err));
    }
...
template <typename... Arguments>
void recordLaunch(
    char* tag, int gs, int bs, void (*f)(Arguments...), Arguments... args)
{
    size_t mem = 0;
...
template <typename... Arguments>
__device__ void
recordLaunch(char* tag, void (*f)(Arguments...), Arguments... args)
{
    clock_t start, stop;
...
// template <typename... Arguments>
// __device__ void recordLaunch(char* tag, void (*f)(Arguments...),
// Arguments&&... args) {
//     clock_t start = clock();
...
__device__ Fun
recordLaunch(char* tag, Fun (*f)(Arguments...), Arguments... args)
{
