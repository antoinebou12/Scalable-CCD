...
#include <scalable_ccd/stq/cuda/sweep.cuh>
#include <scalable_ccd/stq/cuda/timer.cuh>
#include <scalable_ccd/stq/cuda/memory.cuh>
#include <scalable_ccd/stq/cuda/util.cuh>
 ...
 * @param memhandle Memory handler
    ...
    const int memlimit)
{
    ...
    cudaMemcpy(
        d_memhandle, memhandle, sizeof(MemHandler), cudaMemcpyHostToDevice);
    ...
    //              MemHandler *>("runSTQ", grid_dim_1d, threads, runSTQ, d_sm,
    //                            d_mini, d_overlaps, N, d_count, d_start,
    //                            d_memhandle);
    // write recordLaunch for SAPVanilla
    recordLaunch<Scalar2*, const MiniBox*, int2*, int, int*, int*, MemHandler*>(
        "runSAPVanilla", grid_dim_1d, threads, runSAPVanilla, d_sm, d_mini,
        d_overlaps, N, d_count, d_start, d_memhandle);
    ...
    int realcount;
    gpuErrchk(cudaMemcpy(
        &realcount, &(d_memhandle->realcount), sizeof(int),
        cudaMemcpyDeviceToHost));
    spdlog::trace(
    ...
    while (count > memhandle->MAX_OVERLAP_SIZE) {
        ...
        gpuErrchk(cudaMalloc(
            (void**)&d_overlaps, sizeof(int2) * (memhandle->MAX_OVERLAP_SIZE)));
        ...
        // recordLaunch<Scalar2 *, const MiniBox *, int2 *, int, int *, int *,
        //              MemHandler *>("runSTQ", grid_dim_1d, threads, runSTQ,
        //              d_sm,
        //                            d_mini, d_overlaps, N, d_count, d_start,
        //                            d_memhandle);
        recordLaunch<
            Scalar2*, const MiniBox*, int2*, int, int*, int*, MemHandler*>(
            "runSAPVanilla", grid_dim_1d, threads, runSAPVanilla, d_sm, d_mini,
            d_overlaps, N, d_count, d_start, d_memhandle);
    ...
    gpuErrchk(cudaMemcpy(&count, d_count, sizeof(int), cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(
        d_count, &(d_memhandle->realcount), sizeof(int),
