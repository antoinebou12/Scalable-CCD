    ...
    Record& r)
{
    ...
    while ((remain = size - start_id) > 0
        ...
        int tmp_nbr;
        while (overflow) {
            tmp_nbr = std::min(remain, memory_handler->MAX_QUERIES);

            spdlog::debug("itr {:d}", itr);
            if (itr == 0) {
                memory_handler->handleNarrowPhase(tmp_nbr);
            } else {
                memory_handler->handleOverflow(tmp_nbr);
            }
            ...
            // toi = 1;
            run_memory_pool_ccd(
                d_vf_data_list, memory_handler, vf_size,
            ...
            r.Start("run_memory_pool_ccd (narrowphase)", /*gpu=*/true);
            run_memory_pool_ccd(
                d_ee_data_list, memory_handler, ee_size,
    ...
    Scalar& toi)
{
    ...
    spdlog::info("LimitGB {:d}", memory_handler->limitGB);
}
